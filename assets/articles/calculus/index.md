...
**定义**（拓扑子空间）给定一拓扑空间$(S,\tau)$以及$S$的一个子集$X$，我们称$\tau_X=\{V\cap X:V\in\tau\}$为由$(S,\tau)$导出的$X$上的拓扑，并称$(X,\tau_X)$为$(S,\tau)$的一个拓扑子空间
...
**命题**（利用基判断集合是否是开集）设$(S,\tau)$为拓扑空间，$\mathcal B$为它的一个拓扑基，$U$是$S$的子集，$U$是开集当且仅当对任给的$p\in U$，都存在$B\in\mathcal B$使得$p\in B\subseteq U$成立
...
**定理**（Hausdorff空间中收敛的唯一性）设$(S,\tau)$为Hausdorff空间，$\{x_n\}$为其上的点列，诺该点列同时收敛于$p_1$和$p_2$，则$p_1=p_2$
...
**定义**（拓扑流形）设$(M,\tau)$为第二可数的Hausdorff空间，诺存在一个非负整数$n$满足，对空间中的任意一点$p\in S$，都存在一个邻域$U$，使得$U$上的拓扑子空间和$\mathbb R^n$中某个开集上的拓扑子空间同胚，那么我们称$(M,\tau)$是$n$维拓扑流形

**定义**（$n$维单位球面）设$n$为一非负整数，我们定义$n$维单位球面$\mathbb S^n$为集合$\{p\in \mathbb R^{n+1}:\|p\|=1\}$

零维球面就是两个点$\{-1,1\}$，一维球面即圆心在原点的单位圆，二维球面是$\mathbb R^3$中球心在原点的单位球面，也即一般意义下的单位球面。

**推论**（$n$维球面是$n$维拓扑流形）对任意的非负整数$n$，$\mathbb S^n$是$\mathbb R^{n+1}$的子集，由$\mathbb R^{n+1}$导出的$\mathbb S^n$上的拓扑子空间是$n$维拓扑流形

**定理**（维度不变性）给定一个非空的$n$维拓扑流形，那么它只能是$n$维的

**注**：维度不变性只对非空拓扑流形成立，对空拓扑空间$(\emptyset,\{\emptyset\})$而言，它可以是任意维的（为什么？）。

**推论**（拓扑流形中开集上的子空间还是拓扑流形）设$(M,\tau)$为$n$维拓扑流形，$U$为其上一开集，那么$U$上的拓扑子空间是$n$维拓扑流形

**定义**（$n$维半空间）对任意的非负整数$n$，定义$n$维半空间$\mathbb H^n$为集合$\{(x_1,...,x_n)\in\mathbb R^n:x_n\ge 0\}$

**注**：$\mathbb H^0=\mathbb R^0=\{()\}$

**定义**（坐标卡）设$(S,\tau)$为拓扑空间，$U$为其上的开集，诺存在非负数$n$和$\mathbb R^n$或$\mathbb H^n$中的开集$V$，使得$U$上的拓扑子空间和$V$上的拓扑子空间之间存在同胚映射$\varphi:U\to V$，那么我们称$(U,\varphi)$为拓扑空间$(S,\tau)$上的一个坐标卡，此外，诺$V$是$\mathbb R^n$中的开集，那么我们称坐标卡$(U,\varphi)$是内坐标卡，否则称为边坐标卡
...
**定义**（带边流形）设$(M,\tau)$为第二可数的Hausdorff空间，诺存在一个非负整数$n$，使得对空间中的任意一点$p\in M$都存在包含$p$的$n$维坐标卡，那么我们称$(M,\tau)$是$n$维带边流形

显然拓扑流形是一种特殊的带边流形。

**定义**（流形内点与流形边界点）设$(M,\tau)$为$n$维带边流形，$p\in M$为其上一点，诺存在一个包含$p$的内坐标卡，那么称$p$是一个流形内点，诺存在一个包含$p$的边坐标卡$(U,\varphi)$且有$\varphi(p)\in\partial\mathbb H^n$，那么称$p$是一个流形边界点

**注**：

**定理**（流形边界点的拓扑不变性）设$(M,\tau)$为带边流形，$p\in M$为其上一点，那么$p$要么是流形内点，要么是流形边界点，但绝不可能即是内点又是边界点

**命题**（带边流形的一些性质）设$(M,\tau)$为$n$维带边流形，那么
1. $int(E)$是$M$中的开集，且$int(E)$上的拓扑子空间是$n$维拓扑流形
2. $\partial E$是$M$中的闭集，且当$n\ge 1$时，$\partial E$上的拓扑子空间是$n-1$维拓扑流形
3. $M$是拓扑流形当且仅当$\partial M=\emptyset$
...
1. $\phi$的定义域是$\varphi_i(U_i\cap U_j)$，值域是$\varphi_j(U_i\cap U_j)$，它们都是$\mathbb R^n$的子集
...
**定义**（光滑带边流形）诺$(M,\tau)$为带边流形，$\mathcal A$为其上的一个光滑结构，那么我们称$(M,\tau,\mathcal A)$为一个光滑带边流形，特别的，诺$(M,\tau)$为拓扑流形，那么光滑带边流形$(M,\tau,\mathcal A)$也称为光滑流形
