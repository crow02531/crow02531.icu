...
**定义**（Hausdorff空间）诺一个拓扑空间$(S,\tau)$满足：任取空间中不相同的两点$x,y\in S$，存在包含$x$的开集$U_x$和包含$y$的开集$U_y$，使得$U_x\cap U_y=\emptyset$. 那么称该拓扑空间$(S,\tau)$为Hausdorff空间.

**命题**（可度量化拓扑空间是Hausdorff的）设$(S,\tau)$为可度量化拓扑空间，那么$(S,\tau)$是Hausdorff的.

**命题**（Hausdorff空间中收敛的唯一性）设$(S,\tau)$为Hausdorff空间，$\lbrace x_n\rbrace$为其上的点列，诺该点列同时收敛于$p_1$和$p_2$，则$p_1=p_2$.
