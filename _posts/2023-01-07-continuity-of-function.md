...
**例子**：令$d_{l^2}$、$d_{l^1}$分别表示$\mathbb R^2$的欧几里得度量和出租车度量，那么$(\mathbb R^2,d_{l^1})$导出的拓扑等于$(\mathbb R^2,d_{l^2})$导出的拓扑.

**证明**：略.

接下来将介绍一些拓扑空间中常用的概念，由于度量空间中的开集天然构成了该空间的拓扑，这些概念对度量空间也适用.
