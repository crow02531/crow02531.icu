...
接下来将介绍一些拓扑空间中常用的概念. 由于度量空间中的开集天然构成了该空间的拓扑，所以虽然这些概念是定义在拓扑空间上的，但对度量空间也适用.

**定义**（邻域）设$(S,\tau)$为拓扑空间，$p\in S$为其上一点，称$N\subseteq S$是$p$的邻域，当且仅当存在开集$U$满足$p\in U\subseteq N$.

**注**：依照定义，诺$V$是包含$p$的开集，那么$V$是$p$的邻域.

度量空间作为拓扑空间对待时，使用的拓扑是该度量空间导出的拓扑，*度量空间可视为带度量结构的拓扑空间*. 对度量空间而言，空间中某一点的邻域即该点在导出拓扑空间中的邻域.

**命题**（度量空间中的邻域）设$(S,d)$为度量空间，$p\in S$，$N\subseteq S$，以下几个命题是等价的：
1. $N$是$p$的邻域.
2. 令$\tau$表示导出的拓扑，$N$是$p$在$(S,\tau)$中的邻域.
3. 存在开集$U$使得$p\in U\subseteq N$.
4. 存在$r>0$使得$B(p,r)\subseteq N$.

前三点之间的等价性应该是不言而喻的，至于第四点，还请读者自行分析.
...
**命题**（度量空间中的内点、外点、边界点）设$(S,d)$是度量空间，$E\subseteq S$，$p\in S$，那么：
1. 令$\tau$表示导出的拓扑，$p$是$E$的内点（外点、边界点），当且仅当$p$是$E$在$(S,\tau)$中的内点（外点、边界点）.
2. $p$是$E$的内点，当且仅当存在$r>0$，使得$B(p,r)\subseteq E$.
3. $p$是$E$的外点，当且仅当存在$r>0$，使得$B(p,r)\cap E=\emptyset$.
4. $p$是$E$的边界点，当且仅当对任意$r>0$，都有$B(p,r)\cap E\ne\emptyset$.

**证明**：

读者或许已经发现了，在度量空间中，很多有关“包含某点的开集”的描述都可以用“以该点为中心的度量球”等价替代.
